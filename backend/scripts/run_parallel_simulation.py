...
- After completing the simulation, the environment is not immediately closed; it enters command-waiting mode
...
# This fixes issues where the OASIS third-party library reads files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard output streams to UTF-8 (fix console garbled text)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')

    # Force set default encoding (affects default encoding of the open() function)
...
# Available Twitter actions (INTERVIEW not included; INTERVIEW can only be triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (INTERVIEW not included; INTERVIEW can only be triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview both platforms simultaneously, return combined results
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Items that need to be interviewed on both platforms
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: The last processed rowid value (using rowid instead of created_at because formats differ between platforms)
    ...
    try:
        ...
        # This avoids issues with different created_at formats (Twitter uses integers, Reddit uses datetime strings)
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keeping only key fields (preserve full content, do not truncate)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    try:
        ...
        # Follow user: supplement the followed user's name
        ...
        # Mute user: supplement the muted user's name
        ...
        # Create comment: supplement the commented post information
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    # Select which LLM to use based on parameter and configuration
...
async def run_twitter_simulation(
    ...
    # Get real agent name mapping from configuration (use entity_name instead of default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # If a configuration doesn't have a certain agent, use OASIS's default name
...
async def main():
    ...
    # Create shutdown event at the start of main function, ensuring the entire program can respond to exit signals
    ...
    # Initialize logging configuration (disable OASIS logging, clean old files)
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: the program does not exit after simulation completes, waiting for interview commands
    When a termination signal is received, the following is needed:
