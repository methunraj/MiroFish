...
# Adding this prefix prevents the Agent from calling tools and makes it respond directly with text
...
def optimize_interview_prompt(prompt: str) -> str:
    ...
    if not prompt:
        return prompt
    # Avoid adding prefix duplicate times
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) are kept in the backend/scripts/ directory, no longer copied to the simulation directory
    ...
    try:
        ...
        # - completed: Run finished, preparation was completed long ago
...
def prepare_simulation():
    ...
    Features:
    - Auto-detect completed preparation to avoid duplicate generation
    - If preparation is complete, return existing results directly
    ...
    4. LLM intelligently generate simulation config (with retry mechanism)
    ...
    try:
        ...
        logger.info(f"Starting /prepare request processing: simulation_id={simulation_id}, force_regenerate={force_regenerate}")
        
        # Check if already prepared (avoid duplicate generation)
        if not force_regenerate:
            logger.debug(f"Checking simulation {simulation_id} Whether preparation is already complete...")
        ...
        # ========== Sync entity count retrieval (before background task starts) ==========
        # This way the frontend can immediately get the expected Agent count after calling prepare
        try:
            logger.info(f"Syncing entity count: graph_id={state.graph_id}")
            reader = EntityReader()
            # Quick read entities (no edge info needed, count only)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # Do not get edge info to speed up
            )
            ...
            logger.warning(f"Sync entity count retrieval failed (will retry in background task): {e}")
        ...
        def run_prepare():
            try:
                ...
                logger.error(f"Preparation task failed: {str(e)}")
        ...
        return jsonify({
            ...
            "data": {
                ...
                "expected_entities_count": state.entities_count,  # Expected Agent total count
...
def get_prepare_status():
    ...
    try:
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Already have completed preparation",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task:
            # Task does not exist, but if simulation_id is provided, check whether preparation is already complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    try:
        ...
        logger.warning(f"Failed to find report for simulation {simulation_id}  failed: {e}")
        return None
...
def get_simulation_history():
    ...
    For homepage history display, returns a simulation list with rich info like project name and description
...
def get_simulation_config(simulation_id: str):
    ...
    Returns including:
        ...
        - agent_configs: Each Agent's activity config (activity, posting frequency, stance, etc.)
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script files (general scripts, located in backend/scripts/)
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // Optional: Maximum simulation rounds, to truncate overly long simulations
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, will stop and clean up run logs first
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "force_restarted": true               // Whether it is a force restart
            }
        }
    """
    try:
        ...
        # Intelligently handle status: if preparation is complete, allow restart
        if state.status != SimulationStatus.READY:
            # Check if preparation is complete
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation is complete, checking if there is a running process
...
def get_run_status_detail(simulation_id: str):
    ...
    For frontend real-time activity display
    ...
    try:
        ...
        # Get actions of current round (recent_actions only shows latest round)
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (after completing simulation loop and entering command waiting mode)
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // required, simulation ID
            "prompt": "What is your overall view on this?",  // required, interview question (all Agents use the same question)
...
def close_simulation_env():
    ...
    Send a close environment command to the simulation so it gracefully exits command-waiting mode.
