...
# Get logger
logger = get_logger('parallelworld.api')
...
def build_graph():
    ...
    try:
        ...
        # Get configuration
        graph_name = data.get('graph_name', project.name or 'Parallel World Graph')
        ...
        def build_task():
            build_logger = get_logger('parallelworld.build')
            try:
                ...
                # Wait for LLM processing to complete (check processed status of each episode)
                task_manager.update_task(
                    task_id,
                    message="Waiting for LLM entity extraction...",
                    progress=55
                )
