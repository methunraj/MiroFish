...
class ProductLaunchEngine(SimulationEngine):
            ...
            try:
                ...
                messages = [
                    ...
                    {"role": "user", "content": (
                        ...
                        "- investor (~5%): VCs, analysts, financial stakeholders\n\n"
                        "Each persona needs:\n"
    ...
    def run(self, sim_id: str, population: list[AgentPersona], config: dict) -> None:
        ...
        def _bg():
            ...
            try:
                ...
                logger.info(f"[{sim_id}] Product launch sim completed: {count} actions over {len(LAUNCH_PHASES)} phases")
    ...
    def _viz_network(self, sim_id: str, responses: list[dict]) -> dict:
        ...
        for p in population:
            ...
            nodes.append({
                ...
                "group": p.get("demographics", {}).get("income_level", "unknown"),
            })
