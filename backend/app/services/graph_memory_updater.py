...
class GraphMemoryUpdater:
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                # Add episode and immediately extract entities
                ep_uuid = self.store.add_episode(self.graph_id, combined_text)
                self.extractor.extract_from_episode(ep_uuid, self.graph_id)
