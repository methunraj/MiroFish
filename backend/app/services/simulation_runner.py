...
logger = get_logger('parallelworld.simulation_runner')
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation configuration not found, please call /prepare endpoint first")
    ...
    def register_cleanup(cls):
        ...
        Called when Flask application starts, to ensure all simulation processes are cleaned up on server shutdown
        """
        ...
        # In Flask debug mode, only register cleanup in the reloader subprocess (the actual app process)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: Interview both platforms in dual-platform simulation, return merged results
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                - "twitter": Default to only interviewing Twitter platform
                - "reddit": Default to only interviewing Reddit platform
                - None: In dual-platform simulation, each Agent is interviewed on both platforms
            timeout: Timeout (seconds)
    ...
    def close_simulation_env(
        ...
        Send close environment command to the simulation so it gracefully exits command-waiting mode
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (timeout waiting for response, environment may be shutting down)"
            }
