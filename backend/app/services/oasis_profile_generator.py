...
Converts entities from the graph into the Agent Profile format required by the OASIS simulation platform

Optimizations:
1. Calls graph retrieval to enrich node information
...
class OasisProfileGenerator:
    ...
    Converts entities from the graph into the Agent Profiles required for OASIS simulation

    Optimization features:
    1. Calls graph retrieval to obtain richer context
    ...
    def __init__(
        ...
        # GraphStore for retrieving rich context
    ...
    def generate_profile_from_entity(
        ...
        """
        Generate OASIS Agent Profile from an entity
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use graph hybrid search to obtain rich entity-related information

        There is no built-in hybrid search interface, so edges and nodes are searched separately and results merged.
        Parallel requests are used for efficiency.
        ...
        try:
            ...
            # Process edge search results (GraphStore returns dict)
            all_facts = set()
            ...
            # Process node search results (GraphStore returns dict)
            all_summaries = set()
            ...
            logger.info(f"Graph retrieval completed: {entity_name}, obtained {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes")

        except concurrent.futures.TimeoutError:
            logger.warning(f"Retrieval timed out ({entity_name})")
        except Exception as e:
            logger.warning(f"Retrieval failed ({entity_name}): {e}")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich information retrieved from the graph
        """
        context_parts = []
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if the last quote has no comma or closing bracket after it, the string may be truncated
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed and realistic personas for public opinion simulation, faithfully reproducing existing real-world situations. You must return valid JSON format. All string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, faithfully reproducing existing real-world situations.
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Personal memory (important part of persona, describe the individual's connection to the event and their existing actions and reactions in the event)
...
- Use English (except the gender field must use English male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institution/group entity, faithfully reproducing existing real-world situations.
...
2. persona: Detailed account profile description (2000 words of plain text), must include:
   - Institution basic information (official name, institution type, establishment background, main functions)
   ...
   - Institutional memory (important part of institution persona, describe the institution's connection to the event and its existing actions and reactions in the event)
3. age: Fixed to 30 (virtual age for institution accounts)
4. gender: Fixed to "other" (institution accounts use other to indicate non-personal)
...
- Use English (except the gender field must use English "other")
- age must be integer 30, gender must be string "other"
- Institution account speech should match its identity positioning"""
