...
Implements ReACT pattern-based simulation report generation
...
logger = get_logger('parallelworld.report_agent')
...
class ReportLogger:
    ...
    def log_llm_response(
        ...
        self.log(
            ...
            details={
                ...
                "message": f"LLM response (tool_call: {has_tool_calls}, final_answer: {has_final_answer})"
            }
        )
    ...
    def log_section_content(
        ...
        """Log section content generation completion (only content, does not mean the entire section is complete)"""
        self.log(
    ...
    def log_section_full_complete(
        ...
        The frontend should listen for this log to determine if a section is truly complete and to get the full content
        """
        self.log(
...
class ReportConsoleLogger:
    ...
    def _setup_file_handler(self):
        ...
        loggers_to_attach = [
            'parallelworld.report_agent',
            'parallelworld.graph_tools',
        ]
...
[Broad Search - Get a Full Panorama View]
This tool is used to get the complete panorama of simulation results, especially suitable for understanding how events evolve. It will:
...
This is not LLM simulation, but calling the actual interview interface to get original responses from simulation Agents.
...
4. Calls the /api/simulation/interview/batch interface for real interviews on dual platforms
5. Integrates all interview results and provides multi-perspective analysis

[Use Cases]
- Need to understand event perspectives from different role viewpoints (How do students see it? How does the media see it? What do officials say?)
...
PLAN_SYSTEM_PROMPT = """\
You are a "Future Prediction Report" writing expert, with a "god's-eye view" of the simulated world -- you can observe every Agent's behavior, speech, and interactions in the simulation.

[Core Philosophy]
We have built a simulated world and injected specific "simulation requirements" as variables. The evolution results of the simulated world are predictions of what may happen in the future. What you are observing is not "experimental data" but a "preview of the future."
...
- Agent behavior and speech in the simulated world are predictions of future population behavior
- Not an analysis of the current state of the real world
- Not a generic public opinion overview

[Section Count Limits]
- Minimum 2 sections, maximum 5 sections
- No sub-sections needed; each section should be written as complete content directly
...
Note: The sections array must have between 2 and 5 elements!"""
...
1. Under our set conditions, what kind of state did the future present?
...
1. [Must call tools to observe the simulated world]
      ...
      - All content must come from events and Agent behavior/speech in the simulated world
      - You must not use your own knowledge to write report content
      - Call tools at least 3 times (maximum 5 times) per section to observe the simulated world, as it represents the future
...
3. [Language consistency - Write in English]
      - Tool return content may contain mixed languages
      - The report must be written entirely in English
      - When quoting tool-returned content, translate any non-English text into fluent English before writing it into the report
      - Preserve the original meaning while ensuring natural and smooth expression
...
Twitter served as the primary venue for public opinion, fulfilling the core function of information dissemination:

> "Twitter contributed 68% of the initial buzz..."
...
[Tool Usage Recommendations - Mix different tools, do not only use one type]
- insight_forge: Deep insight analysis, automatically breaks down questions and retrieves facts and relationships across multiple dimensions
...
Each response you can only do ONE of the following two things (cannot do both at the same time):

Option A - Call a tool:
Output your thought, then call one tool using the following format:
<tool_call>
...
4. [Quote format specification - Must be standalone paragraphs]
   Quotes must be standalone paragraphs with a blank line before and after, and cannot be mixed within paragraphs:

   Correct format:
   ```
   The official response was considered to lack substantive content.

   > "The official response pattern appeared rigid and sluggish in the ever-changing social media environment."
   ...
   Incorrect format:
   ```
   The official response was considered to lack substantive content.> "The official response pattern..." This evaluation reflects...
   ```
...
REACT_INSUFFICIENT_TOOLS_MSG = (
    ...
    "Please call tools to obtain more simulation data, then output Final Answer. {unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "You have currently only called tools {tool_calls_count} times, at least {min_tool_calls} calls are required."
...
class ReportAgent:
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "Interview topic or requirement description (e.g.: 'Get student opinions on dormitory formaldehyde incident')",
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Broad search - Get full panorama
                query = parameters.get("query", "")
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback - LLM directly outputs bare JSON (not wrapped in tags)
        # Only try when format 1 did not match, to avoid false matches with JSON in body text
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    # Third time: degrade to truncating at first tool call, force execute
                    logger.warning(
                        f"Section {section.title}: {conflict_retries} consecutive conflicts, "
                        "degrading to truncating and executing first tool call"
                    )
            ...
            # Directly use this content as the final answer, no more empty iterations
    ...
    def generate_report(
        ...
        Each section is saved to a folder immediately after generation, no need to wait for the entire report to complete.
        File structure:
        reports/{report_id}/
...
class ReportManager:
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if it is a duplicate of the section title (skip duplicates within first 5 lines)
                if i < 5:
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Keep report main heading (#) and section headings (##), remove other level headings (###, ####, etc.)
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if it is a duplicate heading (same content heading within 5 consecutive lines)
                is_duplicate = False
                ...
                # - # (level=1) only keep report main heading
                ...
                if level == 1:
                    ...
                    # Headings at ### level and below converted to bold text
                    processed_lines.append(f"**{title}**")
                    processed_lines.append("")
                    prev_was_heading = False
