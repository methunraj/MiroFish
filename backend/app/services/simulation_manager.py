...
logger = get_logger('parallelworld.simulation')
...
class SimulationState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Complete status dictionary (for internal use)"""
        return {
...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No entities matching criteria found, please check if the graph was built correctly"
                self._save_simulation_state(state)
                return state
            ...
            if progress_callback:
                progress_callback(
                    ...
                    f"Complete, total {len(profiles)} profiles",
                    current=len(profiles),
                    total=len(profiles)
                )
            
            # ========== Stage 3: LLM intelligent config generation ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0, 
            ...
            # Note: Run scripts are kept in the backend/scripts/ directory, no longer copied to simulation directory
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                f"1. Activate conda environment: conda activate ParallelWorld\n"
