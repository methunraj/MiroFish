...
Fully automated, no manual parameter configuration needed
...
class TimeSimulationConfig:
    ...
    # Off-peak hours (0:00-5:00, almost no activity)
...
class SimulationConfigGenerator:
    ...
    def generate_config(
        ...
        reasoning_parts.append(f"Agent configuration: Successfully generated {len(all_agent_configs)} agents")
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # Leave 500 character buffer
...
- User group follows typical daily activity patterns
- 0:00-5:00 almost no activity (activity multiplier 0.05)
- 6:00-8:00 gradually active (activity multiplier 0.4)
- 9:00-18:00 moderately active during work hours (activity multiplier 0.7)
- 19:00-22:00 is peak time (activity multiplier 1.5)
...
- **Important**: The example values below are for reference only. You need to adjust specific time periods based on event nature and participant group characteristics
  - For example: Student group peak may be 21:00-23:00; media active all day; official institutions only active during work hours
...
- peak_hours (int array): Peak time periods, adjust based on event participant groups
- off_peak_hours (int array): Off-peak time periods, usually late night and early morning
- morning_hours (int array): Morning time periods
- work_hours (int array): Work time periods
- reasoning (string): Brief explanation of why this configuration"""

        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration should match typical daily activity patterns."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # Almost no one active in late night
...
- Design initial post content, **each post must specify a poster_type (poster type)**
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        # Type alias mapping (handles different formats the LLM might output)
        type_aliases = {
...
- **Time should match typical daily activity patterns**: almost no activity 0-5am, most active 7-10pm
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours, considering typical daily activity patterns>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configuration should match typical daily activity patterns."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent configuration by rules (typical daily activity patterns)"""
