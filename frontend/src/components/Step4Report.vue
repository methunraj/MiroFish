                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  <div class="loading-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <circle cx="12" cy="12" r="10" stroke-width="4" stroke="var(--text-faint)"></circle>
                      <path d="M12 2a10 10 0 0 1 10 10" stroke-width="4" stroke="var(--text-dim)" stroke-linecap="round"></path>
                    </svg>
                  </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, but section may not be complete yet) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // Wait for DOM update, then adjust scroll position to keep button in same place
...
const toolConfig = {
  ...
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'accent',
    icon: 'database' // database icon - entity
  }
}
...
const getToolColor = (toolName) => {
  return toolConfig[toolName]?.color || 'accent'
}
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract reasoning for interviewee selection
    const reasonMatch = text.match(/### Reasoning for Interviewee Selection\n([\s\S]*?)(?=\n---\n|\n### Interview Transcripts)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **Alumni_345 (index=1)**: As a WHU alumni...
        ...
        if (name) {
          ...
          // Continuation line of reasoning (exclude trailing summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform tags (very old format), treat as whole answer
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or > unicode quotes
        if (!quoteMatches) {
    ...
    const summaryMatch = text.match(/### Interview Summary and Key Perspectives\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s+related\s+items/)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check if this is platform placeholder text
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" format (backend new format)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 (## xxx) since section title is already shown in outer layer
  ...
  // Clean <p><br> before block-level elements (caused by extra blank lines)
  ...
  // Fix non-continuous ordered list numbering: when single <ol> items are separated by paragraph content, keep numbering incrementing
...
.section-number {
  font-family: var(--font-mono);
  font-size: 16px;
  color: var(--text-faint); /* dark gray, does not change with status */
  font-weight: 500;
}
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: var(--border); padding-bottom: 8px; }
...
.right-panel {
  ...
  --wf-border: var(--text-faint);
  --wf-divider: var(--bg-surface);

  --wf-active-bg: var(--bg-surface);
  --wf-active-border: var(--text);
  --wf-active-dot: var(--text);
  --wf-active-text: var(--text);

  --wf-done-bg: var(--bg-surface);
  --wf-done-border: var(--text-faint);
...
.tool-badge.tool-accent {
  background: var(--bg-raised);
  border-color: var(--accent-dim);
  color: var(--accent);
}
.tool-badge.tool-accent .tool-icon {
  stroke: var(--accent);
}
...
.tool-badge.tool-success .tool-icon {
  stroke: var(--success);
}

/* Tool Colors - Orange (Quick Search) */
.tool-badge.tool-warning {
  background: var(--bg-raised);
  border-color: var(--accent-dim);
  color: var(--warning);
}
.tool-badge.tool-warning .tool-icon {
  stroke: var(--warning);
}
...
.log-content::-webkit-scrollbar-thumb { background: var(--text-dim); border-radius: 0; }
